...
⚠️ 커스텀 구현 이유:
Firebase MCP 서버는 SSE(Server-Sent Events)를 지원하지 않아서 
Google ADK의 표준 MCPToolset을 사용할 수 없습니다.
따라서 HTTP 직접 호출 방식으로 JSON-RPC 2.0 프로토콜을 구현했습니다.

🔧 주요 특징:
- ADK 표준 MCPToolset 대신 직접 HTTP 클라이언트 구현
- JSON-RPC 2.0 프로토콜 수동 처리
- SSE 형식 응답 파싱 지원
- 세션 관리 및 재사용 최적화
- Firebase MCP 서버 특성에 맞춘 헤더 및 파라미터 처리

📋 ADK 호환성:
ADK 표준을 따르지 않는 것이 아니라, Firebase MCP의 제약사항으로 인해
불가피하게 커스텀 구현한 것입니다.

🎯 ADK 표준 구조에서의 역할:
- 인프라 계층: MCP 서버와의 통신 담당
- 도구 지원: Firebase/Email 에이전트들이 공통으로 사용
- 세션 관리: ADK 세션과 MCP 세션 동기화
"""
